...
def tail_command(filepath, lines=10):
    try:
...
def rm_command(filepath):
    try:
        ...
        return "Cannot delete a directory with 'rm'. Use 'rmdir' for directories."
    except OSError as e:
...
def load_vfs():
    ...
    if config['DEFAULT']['vfs_path']:
        try:
            with tarfile.open(config['DEFAULT']['vfs_path'], 'r') as tar_ref:
                ...
                # Call safe_extract with a filter
                safe_extract(tar_ref, "./vfs")
            ...
            print(f"Error: Invalid tar file. {e}")
        except Exception as e:
